...
This file provides guidance to any coding agent (e.g., Claude Code) when working with code in this repository.
...
- **ServiceDefaults**: Aspire-style cross-cutting host defaults — OpenTelemetry (Azure Monitor exporter), default health checks, resilience handlers. Exposes `AddServiceDefaults()` / `AddDefaultHealthChecks()` / `MapDefaultEndpoints()` (`/health`, `/alive`). Consumed by Api and AppHost.
...
dotnet test tests/NetCoreAngularApp.Template.Domain.Tests.Unit
